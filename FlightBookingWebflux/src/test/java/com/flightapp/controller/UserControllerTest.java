...
class UserControllerTest {
    ...
    void testUserLogin_Failure() {
        ...
        StepVerifier.create(userController.userLogin(user))
                .expectNext("Invalid credentials")
                .verifyComplete();
    }
    ...
    void testSearchFlights() {
        ...
        when(flightService.findByFromPlaceAndToPlaceAndDepartureTimeBetween(
                request.get("fromPlace"),
                request.get("toPlace"),
                request.get("departureTime"),
                request.get("arrivalTime")))
                .thenReturn(Flux.just(flight));

        StepVerifier.create(userController.searchFlights(request))
                .expectNext(flight)
                .verifyComplete();
    ...
    void testBookTicket_Success() {
        ...
        StepVerifier.create(userController.bookTicket(Mono.just(ticket)))
                .expectNextMatches(response -> response.getBody().get("pnr").equals("PNR123"))
                .verifyComplete();

        verify(ticketService, times(1))
                .bookTicket("U1", "F1", null, List.of(), FlightType.ONE_WAY);
    }
    ...
    void testGetTicket_Success() {
        ...
        StepVerifier.create(userController.getTicket("PNR123"))
                .expectNextMatches(response -> response.getBody().equals(ticket))
                .verifyComplete();
    ...
    void testCancel_Success() {
        ...
        StepVerifier.create(userController.cancel("PNR123"))
                .expectNextMatches(response ->
                        response.getBody().get("message").equals("Ticket cancelled"))
                .verifyComplete();
